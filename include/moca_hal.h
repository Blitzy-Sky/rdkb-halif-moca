 ...
 * in the repository specification (`docs/pages/halSpec.md`). Replacing these macros with
 * an enumeration would change the type of the constants and so is a functional change,
 * outside the scope of a documentation change set.
 */
...
#if 0
/**
 * @brief Retired enumeration of ACA outcome values, retained here as a reference only.
 *
 * This definition is inside an `#if 0` block, so it is not part of the interface: no
 * declaration uses the type and no caller can name it. It is documented because it
 * records the five outcomes that the surviving contract expresses as untyped integers in
 * the stat member of moca_aca_stat_t - 0 success, 1 bad channel, 2 no EVM probe, 3
 * failure and 4 in progress - and that member's comment is the authority a caller uses.
 * Removing the block, or restoring the type and giving stat that type, would change the
 * interface and so is out of scope for a documentation change.
 *
 * @see moca_aca_stat_t
 */
typedef enum
 ...
 *          build variant as shipped is ill-formed.
 *          Correcting it means either guarding the declaration to match this type or
 *          removing the guard from this type, and both change the set of declarations
 *          the header presents. That is a functional change, and this change set is
 *          documentation only, so the defect is recorded here rather than repaired here.
 *          It is recorded for a reader in the same terms under `Platform or Product
 *          Customization` in the repository specification (`docs/pages/halSpec.md`).
...
typedef struct
{
    ...
    INT stat;                  /**< Outcome of the run, as an untyped INT taking 0 for success, 1 for a bad channel, 2 for a missing EVM probe, 3 for failure and 4 for a run still in progress. It is deliberately documented as five integer values and not as an enumeration - the retired ACA_STATUS type above this structure sits inside an `#if 0` block - so the compiler cannot check a comparison, and this interface neither defines nor reserves a sixth value. It is also the only way to separate a run in progress from a failed request, because the STATUS_INPROGRESS and STATUS_FAILURE return codes share the value -1. */
 ...
 * The review that accompanied this header's migration to open source recorded the
 * intention to extend the set with the specific reasons for a failure. That extension has
 * not been made, and making it would add values to the interface, which is a functional
 * change and outside the scope of a documentation change set.
 *
 ...
 *          not that this one API is unavailable in a `MOCA_VAR` build: that build variant
 *          as shipped is ill-formed, and guarding the caller's own dynamic-information
 *          path under `#ifndef MOCA_VAR` does not avoid it, because the failure happens
 *          inside the header before any of the caller's code is seen. Repairing it means
 *          either guarding this declaration or unguarding the type, and either changes the
 *          set of declarations this header presents - a functional change, so the defect is
 *          recorded here and under `Platform or Product Customization` in the repository
 *          specification rather than repaired in a documentation change set.
 *
 * @see moca_dynamic_info_t
